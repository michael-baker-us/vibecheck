/**
 * Standalone Control Center webview markup.
 *
 * This module intentionally has no `vscode` dependency so the compiled output can be
 * loaded by tooling (see `docs/screenshots`) to render the panel outside the extension host.
 */
export function controlCenterHtml(cspSource: string): string {
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .review-terminal-head { position:sticky; top:0; z-index:1; display:flex; align-items:center; justify-content:space-between; gap:8px; padding:7px 9px; border-bottom:1px solid var(--panel-border); background:var(--panel-surface-strong); font-size:11px; }
    .review-terminal-heading { display:grid; gap:2px; min-width:0; }
    .review-terminal-heading span { color:var(--panel-muted); }
    .review-terminal-actions { display:flex; align-items:center; flex-wrap:wrap; justify-content:flex-end; gap:4px; flex:none; }
    .review-terminal.compact { max-height:none; overflow:visible; }
    .review-terminal.compact .review-terminal-head { position:static; border-bottom:0; }
...
<script nonce="${nonce}">
  ...
  let latestPayload,pendingFocusKey;
  const transcriptView = (key,defaultView) => vscode.getState()?.transcriptViews?.[key]||defaultView;
  const setTranscriptView = (key,view) => { const transcriptViews={...(vscode.getState()?.transcriptViews||{}),[key]:view}; while(Object.keys(transcriptViews).length>20)delete transcriptViews[Object.keys(transcriptViews)[0]]; vscode.setState({...vscode.getState(),transcriptViews}); pendingFocusKey='transcript:'+key+':'+(view==='expanded'?'minimized':'expanded'); if(latestPayload)render(latestPayload); };
  const transcriptButton = (label,key,view) => { const node=el('button','btn small ghost',label); node.type='button'; node.dataset.focusKey='transcript:'+key+':'+view; node.onclick=()=>setTranscriptView(key,view); return node; };
  const cliTranscript = (key,title,detail,entries,defaultView='expanded',pinned=true) => { const view=transcriptView(key,defaultView),terminal=el('div','review-terminal'+(view==='expanded'?'':' compact')),head=el('div','review-terminal-head'),heading=el('div','review-terminal-heading'),actions=el('div','review-terminal-actions'); terminal.dataset.transcriptKey=key; heading.append(el('strong','',view==='cleared'?'CLI preview cleared':title),el('span','',view==='cleared'?'Transcript hidden for this session':detail)); if(view==='expanded')actions.append(transcriptButton('Minimize',key,'minimized'),transcriptButton('Clear preview',key,'cleared')); else actions.append(transcriptButton('Show CLI preview',key,'expanded')); if(view==='minimized')actions.append(transcriptButton('Clear preview',key,'cleared')); head.append(heading,actions); terminal.append(head); if(view==='expanded'){ entries.forEach(entry=>{ const row=el('div','review-terminal-entry '+entry.kind),meta=el('div','review-terminal-meta'); meta.append(el('span','review-terminal-kind',entry.kind),el('time','',new Date(entry.at).toLocaleTimeString([], {hour:'2-digit',minute:'2-digit',second:'2-digit'})),el('strong','',entry.label)); row.append(meta); if(entry.content)row.append(el('pre','',entry.content)); terminal.append(row); }); if(pinned)requestAnimationFrame(()=>{ terminal.scrollTop=terminal.scrollHeight; }); } return terminal; };
  ...
  const unrecognizedText = gate => (gate.reportPathMissing?'Its report file was not found after the run, and no':'No')+' metrics format was recognised in this output. Set format: or report_path: for this check in .vibecheck/config.yaml.';
  const gateOutcome = gate => { if(gate.summary)return summaryText(gate.summary); if(gate.summaryUnrecognized&&(gate.status==='passed'||gate.status==='failed'||gate.status==='stale'))return (gate.status==='failed'?'Failed'+(gate.exitCode===undefined?'':' with exit code '+gate.exitCode)+'. ':'Completed successfully. ')+unrecognizedText(gate); if(gate.status==='passed')return 'Completed successfully. Open the report for command details and raw output.'; if(gate.status==='failed')return 'Failed'+(gate.exitCode===undefined?'':' with exit code '+gate.exitCode)+'. Open the report for diagnostic highlights and raw output.'; if(gate.status==='stale')return 'This result is stale because relevant repository inputs changed. Run the gate again.'; if(gate.status==='running')return 'Running now. Results will appear when the command finishes.'; return 'Not run yet. Run this gate to collect evidence.'; };
  ...
  function render(data) {
    ...
    const transcriptPins=Object.fromEntries([...document.querySelectorAll('.review-terminal[data-transcript-key]')].map(terminal=>[terminal.dataset.transcriptKey,terminal.scrollTop+terminal.clientHeight>=terminal.scrollHeight-24]));
    ...
    const reviewState=s.codeReview;
      ...
      const scopeLabel=reviewState.range&&reviewState.range.scope==='commits'?reviewState.range.baseLabel+' → '+reviewState.range.targetLabel:'working tree';
      const reviewMeta=el('div','meta',scopeLabel+' · '+reviewState.model+' · '+reviewState.effort+' effort · started '+new Date(reviewState.startedAt).toLocaleString()+(reviewState.finishedAt?' · finished '+new Date(reviewState.finishedAt).toLocaleTimeString():''));
      ...
      const transcript=data.reviewTranscript||[]; if(transcript.length){ const key='review:'+reviewState.startedAt; review.content.append(cliTranscript(key,reviewState.status==='running'?'Live CLI review':'CLI review transcript',reviewState.status==='running'?'streaming · memory only':'memory only',transcript,reviewState.status==='completed'?'minimized':'expanded',transcriptPins[key]!==false)); }
    ...
    const reviewScope=section('Review scope','Working tree','review:scope',false);
    const savedReview=(vscode.getState()||{}).reviewOptions||{},reviewForm=el('div','form-grid');
    const rvModeField=el('label','form-field'),rvModeLabel=el('span','','Compare'),rvMode=el('select','');
    rvMode.dataset.focusKey='review:mode'; [['working-tree','Working tree changes vs HEAD'],['branches','Source branch \u2192 target branch'],['commits','Two commits or refs']].forEach(([value,label])=>{ const option=el('option','',label); option.value=value; rvMode.append(option); }); rvMode.value=savedReview.mode||'working-tree'; rvModeField.append(rvModeLabel,rvMode);
    const rvSourceField=el('label','form-field'),rvSourceLabel=el('span','','Source branch'),rvSource=el('input',''); rvSource.type='text'; rvSource.dataset.focusKey='review:source'; rvSource.value=savedReview.source||s.headBranch||'HEAD'; rvSourceField.append(rvSourceLabel,rvSource);
    const rvTargetField=el('label','form-field'),rvTargetLabel=el('span','','Target branch'),rvTarget=el('input',''); rvTarget.type='text'; rvTarget.dataset.focusKey='review:target'; rvTarget.value=savedReview.target||'main'; rvTargetField.append(rvTargetLabel,rvTarget);
    const rvFetchField=el('label','check-field'),rvFetch=el('input',''); rvFetch.type='checkbox'; rvFetch.dataset.focusKey='review:fetch'; rvFetch.checked=savedReview.fetchLatest===true; rvFetchField.append(rvFetch,el('span','','Fetch the latest target branch from its remote before comparing'));
    const rvRemoteField=el('label','form-field'),rvRemote=el('input',''); rvRemote.type='text'; rvRemote.dataset.focusKey='review:remote'; rvRemote.value=savedReview.remote||'origin'; rvRemoteField.append(el('span','','Remote'),rvRemote);
    const rvModelField=el('label','form-field'),rvModel=el('select',''); rvModel.dataset.focusKey='review:model';
    [['codex-balanced','Codex \u00b7 '+routing.codexBalanced+' \u00b7 medium effort'],['codex-deep','Codex \u00b7 '+routing.codexDeep+' \u00b7 high effort'],['claude-balanced','Claude \u00b7 '+routing.claudeBalanced+' \u00b7 medium effort'],['claude-deep','Claude \u00b7 '+routing.claudeDeep+' \u00b7 high effort (Recommended)']].forEach(([value,label])=>{ const option=el('option','',label); option.value=value; rvModel.append(option); }); rvModel.value=savedReview.model||'claude-deep'; rvModelField.append(el('span','','Provider and model'),rvModel);
    const reviewOptions=()=>({mode:rvMode.value,source:rvSource.value,target:rvTarget.value,fetchLatest:rvFetch.checked,remote:rvRemote.value,model:rvModel.value});
    const saveReviewOptions=()=>vscode.setState({...vscode.getState(),reviewOptions:reviewOptions()});
    const updateReviewForm=()=>{ const working=rvMode.value==='working-tree',branches=rvMode.value==='branches'; rvSourceField.hidden=working; rvTargetField.hidden=working; rvFetchField.hidden=!branches; rvRemoteField.hidden=!branches||!rvFetch.checked; rvSourceLabel.textContent=branches?'Source branch':'From commit or ref'; rvTargetLabel.textContent=branches?'Target branch':'To commit or ref'; };
    rvMode.onchange=()=>{ updateReviewForm(); saveReviewOptions(); }; rvFetch.onchange=()=>{ updateReviewForm(); saveReviewOptions(); }; [rvSource,rvTarget,rvRemote].forEach(input=>input.oninput=saveReviewOptions); rvModel.onchange=saveReviewOptions; updateReviewForm();
    const reviewRunning=reviewState&&reviewState.status==='running';
    const runScoped=button(reviewRunning?'Review running\u2026':'Run review on this scope','',undefined,'primary'); runScoped.disabled=reviewRunning===true; runScoped.onclick=()=>{ saveReviewOptions(); vscode.postMessage({action:'run-review',options:reviewOptions()}); };
    reviewForm.append(rvModeField,rvSourceField,rvTargetField,rvFetchField,rvRemoteField,rvModelField,runScoped);
    reviewScope.content.append(el('div','section-intro','Review the working tree, a branch against its merge base, or any two commits.'),reviewForm);
    pages.review.append(reviewScope.card);
    pages.review.append(review.card);
    const modelRoutes=section('Model routing','Balanced + Deep','settings:model-routing',false),routingIntro=el('div','section-intro','Choose the exact provider model used by semantic reviews, change summaries, and README maintenance. Effort stays profile-based: Balanced uses medium; Deep uses high.'),routingForm=el('div','form-grid');
    ...
    const saveRoutes=button('Save model routes','',undefined,'primary'); saveRoutes.onclick=()=>vscode.postMessage({action:'set-model-routing',options:Object.fromEntries(Object.entries(routeFields).map(([key,input])=>[key,input.value]))}); routingForm.append(saveRoutes); modelRoutes.content.append(routingIntro,routingForm); pages.settings.append(modelRoutes.card);
    pages.review.append(el('div','eyebrow','Change communication'),el('div','section-intro','Create a plain-language Markdown summary for working-tree, branch, or revision changes.'));
    const changeSummary=section('Change summary','Markdown','review:change-summary',true);
    ...
    const readmeSession=data.readmeMaintenanceSession,readmeRunning=readmeSession?.status==='running',readme=section('README maintenance','Git-aware','review:readme',true);
    ...
    pages.review.append(readme.card);

    const usageState=data.providerUsage,usage=section('Claude & Codex usage',usageState.status==='loading'?'refreshing':'provider reported','settings:usage',false);
    ...
    if(setupSession){ const tone=setupSession.status==='completed'?'ready':setupSession.status==='failed'?'blocked':'incomplete',head=el('div','item-head'); head.append(el('span','item-title',(setupSession.provider==='codex'?'Codex':'Claude')+' · '+setupSession.profile+' configuration '+setupSession.mode),el('span','badge '+tone,setupSession.status)); const item=el('div','item'),changed=setupSession.changedFiles?.length?' · '+setupSession.changedFiles.join(', '):''; item.append(head,el('div','meta',setupSession.model+' · '+setupSession.effort+' effort · started '+new Date(setupSession.startedAt).toLocaleString()+changed)); if(setupSession.error)item.append(el('div','callout danger',setupSession.error)); gates.content.append(item); const transcript=setupSession.transcript||[]; if(transcript.length){ const key='configuration:'+setupSession.startedAt; gates.content.append(cliTranscript(key,setupRunning?'Live CLI configuration':'CLI configuration transcript',setupRunning?'streaming · memory only':'memory only',transcript,setupSession.status==='completed'?'minimized':'expanded',transcriptPins[key]!==false)); } }
    ...
    const agents=section('Agent workspace',s.agentFiles.filter(f=>f.exists).length+' configured','settings:agents',false);
    ...
    alignment.append(alignmentHead,el('p','','Generate instruction files asks the selected CLI to propose AGENTS.md and CLAUDE.md. Generate supporting files separately asks whether this repository benefits from skills, agents, rules, hooks, settings, MCP, or output styles. Both remain read-only until review. Safe alignment keeps the shared import and portable skill directories synchronized; provider-specific formats remain explicit review items.'));
    ...
    const alignmentActions=el('div','item-actions'),generateInstructions=button(instructionRunning?'Scanning repository…':'Generate instruction files','generate-agent-instructions',undefined,'primary'),refreshInstructions=button(instructionRunning?'Scanning repository…':'Generate supporting files','refresh-agent-instructions',undefined,'secondary'),clearAgentWorkspace=button('Clear agent workspace','clear-agent-workspace',undefined,'ghost danger'); generateInstructions.disabled=instructionRunning; refreshInstructions.disabled=instructionRunning; clearAgentWorkspace.disabled=instructionRunning; alignmentActions.append(generateInstructions,refreshInstructions,button('Align safe changes now','align-agent-instructions',undefined,'secondary'),clearAgentWorkspace); alignment.append(alignmentToggle,alignmentActions);
    if(instructionSession){ const tone=instructionSession.status==='applied'?'ready':instructionSession.status==='failed'?'blocked':instructionSession.status==='preview'?'incomplete':'neutral',sessionItem=el('div','callout'),sessionHead=el('div','item-head'); sessionHead.append(el('strong','',(instructionSession.provider==='codex'?'Codex':'Claude')+' · '+instructionSession.profile+' workspace proposal'),el('span','badge '+tone,instructionSession.status)); sessionItem.append(sessionHead,el('div','meta',instructionSession.model+' · '+instructionSession.effort+' effort · started '+new Date(instructionSession.startedAt).toLocaleString())); if(instructionSession.summary)sessionItem.append(el('p','',instructionSession.summary)); if(instructionSession.error)sessionItem.append(el('p','danger',instructionSession.error)); (instructionSession.files||[]).forEach(file=>{ const fileRow=el('div','item'),head=el('div','item-head'); head.append(el('span','item-title',file.path),el('span','badge '+(file.status==='unchanged'?'ready':'incomplete'),file.status)); fileRow.append(head,el('p','',file.rationale)); sessionItem.append(fileRow); }); if(instructionPreview){ const proposalActions=el('div','item-actions'); proposalActions.append(button('Preview all changes','preview-agent-workspace',undefined,'secondary'),button('Apply all proposed files','apply-agent-instructions',undefined,'primary'),button('Discard','discard-agent-instructions',undefined,'ghost')); sessionItem.append(proposalActions); } alignment.append(sessionItem); const transcript=instructionSession.transcript||[]; if(transcript.length){ const key='workspace:'+instructionSession.startedAt; alignment.append(cliTranscript(key,instructionRunning?'Live CLI workspace scan':'CLI workspace scan transcript',instructionRunning?'streaming · memory only':'memory only',transcript,instructionSession.status==='applied'?'minimized':'expanded',transcriptPins[key]!==false)); } }
    const alignmentItems=(data.agentAlignment?.items||[]).filter(item=>item.status!=='not-configured');
    alignmentItems.forEach(entry=>{ const row=el('div','callout'),head=el('div','item-head'),tone=entry.status==='aligned'||entry.status==='shared'?'ready':entry.status==='conflict'?'blocked':'incomplete'; head.append(el('strong','',entry.label),el('span','badge '+tone,entry.status)); row.append(head,el('div','meta',entry.surface+(entry.newer?' · '+entry.newer+' changed more recently':'')),el('p','',entry.detail)); const actions=el('div','item-actions'); if(entry.surface==='plans')actions.append(button('Open plan','open-plan',undefined,'ghost')); else { if(entry.codexPath&&/\\.[^/]+$/.test(entry.codexPath))actions.append(button('Open Codex file','manage-agent-file',entry.codexPath,'ghost')); if(entry.claudePath&&entry.claudePath!==entry.codexPath&&/\\.[^/]+$/.test(entry.claudePath))actions.append(button('Open Claude file','manage-agent-file',entry.claudePath,'ghost')); } if(entry.surface==='skills'&&entry.status==='conflict'){ actions.append(button('Use Codex version','resolve-agent-alignment',entry.id+'|codex','ghost'),button('Use Claude version','resolve-agent-alignment',entry.id+'|claude','ghost')); } if(actions.childElementCount)row.append(actions); alignment.append(row); });
    ...
    const localTools=section('Monitoring & local data','local','settings:local-data',false),row1=el('div','actions'); row1.append(button(s.paused?'Resume monitoring':'Pause monitoring',s.paused?'resume':'pause'),button('Refresh now','refresh'),button('Delete local data','delete',undefined,'ghost danger')); localTools.content.append(row1); pages.settings.append(localTools.card);
