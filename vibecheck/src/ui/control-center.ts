...
<script nonce="${nonce}">
  ...
  function render(data) {
    ...
    const alignmentActions=el('div','item-actions'),initializeInstructions=button('Initialize instructions','initialize-agent-workspace',undefined,'primary'),refreshInstructions=button(instructionRunning?'Scanning repository…':'Generate supporting files','refresh-agent-instructions',undefined,'secondary'),clearAgentWorkspace=button('Clear agent workspace','clear-agent-workspace',undefined,'ghost danger'); initializeInstructions.disabled=instructionRunning; refreshInstructions.disabled=instructionRunning; clearAgentWorkspace.disabled=instructionRunning; alignmentActions.append(initializeInstructions,refreshInstructions,button('Align safe changes now','align-agent-instructions',undefined,'secondary'),clearAgentWorkspace); alignment.append(alignmentToggle,alignmentActions);
