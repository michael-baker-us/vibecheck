...
export class ControlCenterProvider implements vscode.WebviewViewProvider {
  ...
  private async handle(message: WebviewMessage): Promise<void> {
    ...
    if (message.action === "set-agent-alignment" && typeof message.options === "boolean") {
      await vscode.commands.executeCommand("intentLoop.setAgentAlignment", message.options);
      return;
    }
...
<script nonce="${nonce}">
  ...
  function render(data) {
    ...
    const summary=el('div','callout'); summary.append(el('strong','', 'Current evidence snapshot'),el('p','',(s.headSubject?s.headSubject+' · ':'')+s.verification.filter(v=>v.status==='passed').length+' checks passed · '+open.length+' findings open · updated '+new Date(s.lastUpdatedAt).toLocaleTimeString())); evidence.content.append(summary); const ea=el('div','actions'); ea.append(button('Create Markdown report','export'),button('Copy agent follow-up','copy-prompt'),button('Show check output','check-output-menu',undefined,'ghost'),button('Open quality config','config',undefined,'ghost')); evidence.content.append(ea); pages.status.append(evidence.card);
